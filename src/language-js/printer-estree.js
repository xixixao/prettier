...
function blockArgument(path, print, options, body, name) {
  const usesParens = shouldPrintParens(options, body);
  const newLine = usesParens ? softline : "";
  const argument = indent(concat([newLine, path.call(print, name)]));
  return concat([
    openParen(options, body),
    group(concat([usesParens ? argument : indent(argument), newLine])),
  ...
  switch (n.type) {
            ...
            group(
              concat([
                ...
                indent(
                  concat([
                    options.bracketSpacing ? line : softline,
                    join(concat([lineComma(options), line]), grouped)
                  ])
                ),
    ...
    case "WithStatement":
      return group(
        concat([
          "with",
          blockArgument(path, print, options, n.body, "body"),
    ...
    case "IfStatement": {
      ...
      const opening = group(
        concat([
          "if",
          blockArgument(path, print, options, n.consequent, "test"),
          con
        ])
      );
    ...
    case "WhileStatement":
      return group(
        concat([
          "while",
          blockArgument(path, print, options, n.body, "test"),
          adjustClause(n.body, path.call(print, "body"))
        ])
      );
    case "ForInStatement":
    ...
    case "SwitchStatement":
      return concat([
        group(
          concat([
            "switch",
            blockArgument(path, print, options, false, "discriminant")
          ])
        ),
...
function printArgumentsList(path, options, print) {
  ...
  const printedArguments = path.map((argPath, index) => {
    ...
    if (index === lastArgIndex) {
      ...
      parts.push(lineComma(options), line);
    }
...
function printFunctionParams(path, print, options, expandArg, printTypeParams) {
  ...
  return concat([
    ...
    "(",
    indent(
      concat([softline, join(concat([lineComma(options), line]), printed)])
    ),
    ifBreak(
          ...
          ? group(
              concat([
                ...
                indent(
                  concat([
                    options.bracketSpacing ? line : softline,
                    join(concat([lineComma(options), line]), specifiers)
                  ])
                ),
                ifBreak(shouldPrintComma(options) ? lineComma(options) : ""),
  ...
  return group(
    concat([
      ...
      indent(
        concat([
          softline,
          join(concat([lineComma(options), line]), path.map(print, paramsKey))
        ])
      ),
      ifBreak(
...
function printArrayItems(path, options, printPath, print) {
  ...
  path.each(childPath => {
    ...
    separatorParts = [lineComma(options), line];
    if (
      childPath.getValue() &&
      sharedUtil.isNextLineEmpty(
        options.originalText,
